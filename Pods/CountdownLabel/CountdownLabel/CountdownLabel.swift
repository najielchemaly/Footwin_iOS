...
//            text = dateFormatter.string(from: date1970.addingTimeInterval(0) as Date)
    ...
    func getTimeRemaining(date: Date) -> String {
...
extension CountdownLabel {
    ...
    func updateText() {
        ...
        if let countdownAttributedText = countdownAttributedText {
            ...
            text = getTimeRemaining(date: targetDate as Date)//attributedString.string
        } else {
            text = getTimeRemaining(date: targetDate as Date)//formattedText
        }
        setNeedsDisplay()
    }
